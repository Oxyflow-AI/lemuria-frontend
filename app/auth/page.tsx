...
export default function AuthPage() {
  ...
  const handleGoogleAuth = async () => {
    ...
    setTimeout(() => {
      setIsLoading(false)
      router.push("/astrology-selection")
    }, 2000)
  }
    ...
    <div className="min-h-screen mystical-gradient flex flex-col p-4">
      ...
      >
        ...
        <div className="flex items-center">
          ...
          <span className="text-xl font-semibold bg-gradient-to-r from-purple-700 to-pink-600 bg-clip-text text-transparent">
            Lemuria
          </span>
        </div>
      </motion.div>
      ...
      >
        <Card className="w-full max-w-md card-gradient cosmic-glow border-purple-100">
          <CardHeader className="text-center">
            <CardTitle className="text-2xl font-bold bg-gradient-to-r from-purple-700 to-pink-600 bg-clip-text text-transparent">
              Welcome Back
            </CardTitle>
            <CardDescription className="text-purple-600">Sign in to continue your mystical journey</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <Button
              ...
              disabled={isLoading}
              variant="outline"
              className="w-full py-3 input-gradient hover:bg-purple-50 border-purple-200"
            >
              {isLoading ? (
            ...
            <div className="relative">
              <div className="absolute inset-0 flex items-center">
                <Separator className="w-full bg-purple-200" />
              </div>
            ...
            <div className="space-y-4">
              {!otpSent ? (
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-sm text-purple-600 text-center">Code sent to {email}</p>
                </div>
              )}

              <Button
                onClick={handleEmailAuth}
                disabled={isLoading || (!otpSent && !email) || (otpSent && otp.join("").length !== 6)}
